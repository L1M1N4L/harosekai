              ...
              >
                ...
                <div className="space-y-6 sm:space-y-8">
                  {upcomingTalks.map((talk, idx) => (
                    ...
                    >
                      <div className="group relative overflow-hidden rounded-2xl bg-white border border-slate-200/60 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
                        {talk.image ? (
                          ...
                          <div className="h-72 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
                            <div className="text-center p-6">
                              <div className="text-4xl mb-2">📢</div>
                              <div className="text-slate-600 font-medium">{talk.event}</div>
                            </div>
                          </div>
                        )}
                        <div className="p-6">
                          <p className="text-slate-600 text-sm leading-relaxed mb-4">{talk.description}</p>
                          {talk.eventPage && (
                            <a href={talk.eventPage} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-medium text-slate-900 hover:text-slate-700 transition-colors border-b border-slate-300 hover:border-slate-500">
                              Event Page
                ...
                >
                  <div className="group relative overflow-hidden rounded-2xl bg-white border border-slate-200/60 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
                    <div className="flex flex-col lg:flex-row lg:items-stretch">
                      {talk.image ? (
                        <div className="lg:w-80 lg:flex-shrink-0 relative h-72 lg:h-auto overflow-hidden">
                          <img 
                            src={talk.image} 
                            alt={talk.event}
                        ...
                        <div className="lg:w-80 lg:flex-shrink-0 h-72 lg:h-auto bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
                          <div className="text-center p-4">
                            <div className="text-3xl mb-2">🎤</div>
                            <div className="text-slate-600 text-xs font-medium">{talk.event}</div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex-1 p-6 lg:p-8">
                        <div className="flex flex-wrap items-start gap-3 mb-3">
                          <h3 className="text-xl sm:text-2xl font-bold text-slate-900 flex-1 min-w-[200px]">{talk.title}</h3>
                        ...
                        <div className="text-sm text-slate-500 mb-4 font-medium flex items-center gap-2">
                          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                          <div className="flex items-center gap-2 p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                            ...
                            <span className="text-xs text-slate-600 font-medium truncate">{talk.date}</span>
                          </div>
                          <div className="flex items-center gap-2 p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                            <svg className="w-4 h-4 text-slate-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                            </svg>
                            <span className="text-xs text-slate-600 font-medium truncate">{talk.location}</span>
                          </div>
                          <div className="flex items-center gap-2 p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                            <svg className="w-4 h-4 text-slate-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z" clipRule="evenodd" />
                            </svg>
                            <span className="text-xs text-slate-600 font-medium truncate">{talk.attendees}</span>
                          </div>
                          <div className="flex items-center gap-2 p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                            <svg className="w-4 h-4 text-slate-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                            </svg>
                            <span className="text-xs text-slate-600 font-medium truncate">{talk.duration}</span>
                          </div>
                        </div>

                        <div className="flex flex-wrap gap-2 mb-6">
                          {talk.tags.map((tag, tagIdx) => (
                            <span key={tagIdx} className="px-2.5 py-1 text-xs bg-slate-100 text-slate-700 rounded-md border border-slate-200 font-medium">
                              {tag}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3">
                          {talk.eventPage && (
                            <a 
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium rounded-lg transition-all duration-200 hover:shadow-lg"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                              </svg>
                          ...
                          {talk.slides && (
                            <a 
                              ...
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-all duration-200 hover:shadow-lg"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
